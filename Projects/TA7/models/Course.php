...
class Course {
	...
	public function getCoursesHiring() {
		...
		$courses = DB::getInstance()->get('RealCourse', array( 'TAneeded', '=', 'yes'));
		$results = $courses->all();
		$courseresult .= "<h2>Here are all the classes Hiring.</h2>";


		$courseresult .= "<table id = 'HiringTable'><tr>
		<td><b>Department</b></td>
		<td><b>Catalogue Number</b></td>
		<td><b>Course Name</b></td> 
		<td><b>Course Number</b></td> 
		<td><b>Professor</b></td> 
		<td><b>TA Assigned</b></td> 
...

	{
		...
		$taH = $results[$x-1]->taHired ;
		$taP = $results[$x-1]->taProbable ;

		$courseresult .=  
		"<tr><td class='dep'>" . $dep ." </td><td class = 'depnumber'> " . $cat . " </td><td class='cname'> " . $name ." </td><td class = 'cnumber'>"  . $num  . " </td><td class= 'prof'>"  . $prof  . " </td>
		<td>"  . $taH  . " </td></tr>";
		$x--;
	}
